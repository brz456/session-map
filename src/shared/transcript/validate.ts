// src/shared/transcript/validate.ts
// Validate a stored Transcript payload (fail-closed, no mutation)
...
export type TranscriptValidationResult =
  ...
  | { ok: false; code: TranscriptValidationErrorCode; message: string };

function isStringArray(arr: unknown): arr is string[] {
  return Array.isArray(arr) && arr.every((item) => typeof item === 'string');
}

export function validateTranscript(input: unknown): TranscriptValidationResult {
  ...
  // Validate importedAtIso
  if (typeof obj.importedAtIso !== 'string') {
    return { ok: false, code: 'missing_required_field', message: 'importedAtIso must be a string' };
  }
  ...
  for (let i = 0; i < obj.sections.length; i++) {
    ...
    // Required fields
    if (typeof sec.sectionId !== 'string') {
      return { ok: false, code: 'missing_required_field', message: `Section at index ${i} missing sectionId string` };
    }
  ...
  for (let i = 0; i < obj.utterances.length; i++) {
    ...
    if (typeof u.utteranceId !== 'string') {
      return { ok: false, code: 'missing_required_field', message: `Utterance at index ${i} missing utteranceId string` };
    }
    ...
    if (!speakerIds.has(u.speakerId)) {
      return { ok: false, code: 'invalid_input', message: `Utterance at index ${i} has unknown speakerId: ${u.speakerId}` };
    }
  ...
  return { ok: true, transcript: input as Transcript };
}
